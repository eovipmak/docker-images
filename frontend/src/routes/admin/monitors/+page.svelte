<script lang="ts">
    ...
    async function loadMonitors() {
        ...
        error = '';
        try {
            const response = await fetchAPI('/api/v1/admin/monitors');
            if (response.ok) {
    ...
    function getMonitorStatus(monitor: any): 'up' | 'down' | 'unknown' {
         // Assuming monitor object has a status or last_check_status field, 
         // but the admin list might just be the monitor definition. 
         // If status is not available, default to unknown or adjust based on API response structure.
         // For now, let's assume unknown if not present.
         return monitor.status || 'unknown'; 
    }
</script>

<svelte:head>
	<title>System Monitors - Admin - V-Insight</title>
</svelte:head>

<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">System Monitors</h1>
            <p class="text-gray-500 dark:text-gray-400">View all monitors across the system.</p>
        </div>
        <button on:click={loadMonitors} class="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
            ...
            <table class="min-w-full divide-y divide-gray-300 dark:divide-slate-700">
                <thead class="bg-gray-50 dark:bg-slate-900">
                    <tr>
                        ...
                        <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-white">URL</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-slate-700 bg-white dark:bg-slate-800">
                    {#if isLoading}
                        {#each Array(5) as _}
                            <tr>
                                ...
                                <td class="px-3 py-4"><div class="h-4 bg-gray-200 dark:bg-slate-700 rounded w-48 animate-pulse"></div></td>
                    ...
                    {:else if monitors.length === 0}
                         <tr>
                            <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">No monitors found</td>
                        </tr>
                    {:else}
                        {#each monitors as monitor}
                            <tr>
                                ...
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-300">{monitor.user_id}</td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-300">
                                    <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {monitor.enabled ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300'}">
