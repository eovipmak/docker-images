...
/**
 * Setup script for V-Insight frontend
 * Creates necessary directory structures and files for SvelteKit dynamic routes
 */
...
const MONITOR_DETAIL_PAGE_CONTENT = `<script lang="ts">
...
async function setupMonitorDetailPage() {
	const monitorIdDir = join(__dirname, 'src', 'routes', 'domains', '[id]');
	const monitorDetailPage = join(monitorIdDir, '+page.svelte');

	try {
		await mkdir(monitorIdDir, { recursive: true });
		console.log('✓ Created directory: src/routes/domains/[id]');

		if (!(await fileExists(monitorDetailPage))) {
			await writeFile(monitorDetailPage, MONITOR_DETAIL_PAGE_CONTENT, 'utf-8');
			console.log('✓ Created file: src/routes/domains/[id]/+page.svelte');
		} else {
			console.log('→ File already exists: src/routes/domains/[id]/+page.svelte');
		}
